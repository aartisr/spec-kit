...
export const SpecKitVersionSelector: React.FC<SpecKitVersionSelectorProps> = ({
  ...
  return (
    <div className="rounded-2xl bg-zinc-900/80 border border-zinc-800 p-5 md:p-6 space-y-5 shadow-xl">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b border-zinc-800/80 pb-4">
        <div className="space-y-1">
          <div className="flex items-center gap-2">
            <Cpu className="w-5 h-5 text-cyan-400" />
            <h3 className="text-base font-bold text-zinc-100">Spec-Kit Engine & Version Selector</h3>
            <span className="px-2 py-0.5 rounded-full text-[10px] font-extrabold bg-indigo-500/20 text-indigo-300 border border-indigo-500/30 font-mono">
        ...
        <div className="relative shrink-0">
          <select
            ...
            className="w-full sm:w-auto px-4 py-2 rounded-xl bg-zinc-950 border border-zinc-800 text-xs font-semibold text-zinc-200 focus:outline-none focus:border-indigo-500 cursor-pointer"
          >
      ...
      <div className="space-y-3">
        <div className="flex items-center justify-between text-xs">
          ...
          <span className="text-zinc-400 text-[11px]">Released {activeOption.releaseDate}</span>
        </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-2.5 pt-1">
          {activeOption.features.map((feat, idx) => (
            <div
              key={idx}
              className="px-3 py-2 rounded-xl bg-zinc-950/80 border border-zinc-800/80 flex items-center gap-2 text-xs text-zinc-300"
            >
